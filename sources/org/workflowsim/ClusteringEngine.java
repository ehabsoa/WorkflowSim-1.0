...
public final class ClusteringEngine extends SimEntity {
	 ...
	 * @param schedulers
	 *            the number of schedulers in this ClusteringEngine
	 * @throws Exception
	 *             the exception
	 * @pre name != null
	 * @post $none
	...
	protected void processClustering() {
		...
		case HORIZONTAL:
			...
			if (params.getClustersNum() != 0) {
				this.engine = new HorizontalClustering(params.getClustersNum(),
						0);
			}/**
			 * else if clusters.size is set in configuration file
			 */
			else if (params.getClustersSize() != 0) {
				this.engine = new HorizontalClustering(0,
						params.getClustersSize());
			}/**
		 ...
		 */
		case BLOCK:
			this.engine = new BlockClustering(params.getClustersNum(),
					params.getClustersSize());
			break;
		/**
	 ...
	 * @return
	 */
	private boolean isRealInputFile(List<org.cloudbus.cloudsim.File> list,
			org.cloudbus.cloudsim.File file) {
		/**
	...
	protected void processDatastaging() {
		...
		for (Iterator it = list.iterator(); it.hasNext();) {
			org.cloudbus.cloudsim.File file = (org.cloudbus.cloudsim.File) it
					.next();
			/**
			...
			if (isRealInputFile(list, file)) {
				ReplicaCatalog
						.addStorageList(file.getName(), Parameters.SOURCE);
				fileList.add(file);
			}
		}
	...
	public void processEvent(SimEvent ev) {
		...
		case WorkflowSimTags.JOB_SUBMIT:
			...
			setTaskList(list);
			/**
			 * It doesn't mean we must do clustering here because by default the
			 * processClustering() does nothing unless in the configuration file
			 * we have specified to use clustering
			 */
			processClustering();
			/**
			 * Add stage-in jobs Currently we just add a job that has minimum
			 * runtime but inputs all input data at the beginning of the
			 * workflow execution
			 */
			processDatastaging();
	 ...
	 * @param <T>
	 *            the generic type
	 * @param taskSubmittedList
	 *            the new task submitted list
	 */
