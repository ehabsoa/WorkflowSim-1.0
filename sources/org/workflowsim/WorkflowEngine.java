...
public class WorkflowEngine extends SimEntity {
	...
	public WorkflowEngine(String name, int schedulers) throws Exception {
		...
		for (int i = 0; i < schedulers; i++) {
			WorkflowScheduler wfs = new WorkflowScheduler(name + "_Scheduler_"
					+ i);
	...
	public void submitVmList(List<? extends Vm> list) {
		...
		// bug here, not sure whether we should have different workflow
		// schedulers
	...
	protected void submitJobs() {
		...
		for (int i = 0; i < num; i++) {
			...
			if (!hasJobListContainsID(this.getJobsReceivedList(),
					job.getCloudletId())) {
				...
				if (flag) {
					...
					List submittedList = (List) allocationList.get(job
							.getUserId());
		...
		for (int i = 0; i < getSchedulers().size(); i++) {
			...
			if (Parameters.getOverheadParams().getWEDDelay() != null) {
				delay = Parameters.getOverheadParams().getWEDDelay(
						submittedList);
			}
			...
			if (interval > 0 && interval <= size) {
				...
				while (index < size) {
					...
					if (index % interval == 0) {
						// create a new one
						schedule(getSchedulerId(i), delay,
								CloudSimTags.CLOUDLET_SUBMIT, subList);
						delay += delaybase;
				...
				sendNow(this.getSchedulerId(i), CloudSimTags.CLOUDLET_SUBMIT,
						submittedList);
			}
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see cloudsim.core.SimEntity#startEntity() Here we creata a message when
	 * it is started
	 */
	@Override
	 ...
	 * @param <T>
	 *            the generic type
	 * @param jobsSubmittedList
	 *            the new job submitted list
	 */
	private <T extends Cloudlet> void setJobsSubmittedList(
			List<T> jobsSubmittedList) {
	 ...
	 *            the new job received list
	 */
	private <T extends Cloudlet> void setJobsReceivedList(
			List<T> jobsReceivedList) {
